...
app.controllerProvider.register('release-details', ['$scope', '$location', '$http', '$compile',
	...
	$scope.draw = function(){
		...
		var timeDiff = Math.abs(deadLineRelease.getTime() - minDate.getTime());
		...
		//draw vertical lines
		drawVerticalGridLines(interval, numberOfInterval);
		//draw vertical days
		var nextDayDeadLine = new Date($scope.release.deadline);
		nextDayDeadLine.setDate(deadLineRelease.getDate()+1);
		
		var dateArray = getDates(minDate, nextDayDeadLine); 
	...
	function drawVerticalGridLines(interval, numberOfInterval) {
